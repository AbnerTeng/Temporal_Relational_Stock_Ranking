...
class EOD_Preprocessor:
    ...
    def _read_EOD_data(self):
        ...
        for index, ticker in enumerate(self.tickers):
            single_EOD = np.genfromtxt(
                os.path.join(
                    self.data_path, self.market_name + "_" + ticker + "_30Y.csv"
                ),
                dtype=str,
                delimiter=",",
                skip_header=True,
        ...
        assert len(self.tickers) == len(self.data_EOD), (
            "length of tickers and stocks not match"
        )

    def _read_tickers(self, ticker_fname):
        self.tickers = np.genfromtxt(
            ticker_fname, dtype=str, delimiter="\t", skip_header=True
        )[:, 0]
    ...
    def generate_feature(
        self, selected_tickers_fname, begin_date, opath, return_days=1, pad_begin=29
    ):
        trading_dates = np.genfromtxt(
            os.path.join(
                self.data_path, "..", self.market_name + "_aver_line_dates.csv"
            ),
            dtype=str,
            delimiter=",",
            skip_header=False,
        ...
        for index, date in tqdm(enumerate(trading_dates)):
        ...
        self._read_EOD_data()
        for stock_index, single_EOD in tqdm(enumerate(self.data_EOD)):
            # selected_EOD = single_EOD
...
if __name__ == "__main__":
    ...
    #     os.path.join(processor.data_path, "..", "2013-01-01-v2"),
    #     return_days=1,
    #     pad_begin=29,
    # )
